...
resource "libvirt_volume" "ex415-satellite-volume" {
  name   = "libvirt_volume-qcow2-ex415_satellite"
...
# Create the machine
resource "libvirt_domain" "ex415-satellite" {
  ...
  provisioner "local-exec" {
    command = "sudo ansible-playbook ${var.ansible_path}/setup_satellite.yml -i ${self.network_interface[0].addresses[0]},"
  }

  # Specify to attach disk that was defined in top resource block
  disk {
    volume_id = libvirt_volume.ex415-satellite-volume.id
  }
